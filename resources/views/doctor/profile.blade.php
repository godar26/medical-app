...
<div class="max-w-2xl mx-auto">
    <div class="card rounded-2xl p-8 fade-up">
        ...
        <div class="flex items-center gap-5 mb-8 pb-8" style="border-bottom:1px solid rgba(255,255,255,0.07)">
            ...
            <div>
                <h2 class="text-white font-semibold text-lg">Dr. {{ $user->first_name }} {{ $user->last_name }}</h2>
                <p style="color:#2dd4bf" class="text-sm">{{ $user->specialization }}</p>
        ...
        <form method="POST" action="{{ route('doctor.profile.update') }}"
            ...
            <div>
                ...
                <input name="specialization" value="{{ old('specialization', $user->specialization) }}" type="text" required
                       class="w-full px-4 py-2.5 rounded-xl text-sm text-white outline-none transition-all"
                       style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)">
            </div>
            ...
            <div>
                ...
                <input name="consultation_fee" value="{{ old('consultation_fee', $user->consultation_fee) }}" type="number" required
                       class="w-full px-4 py-2.5 rounded-xl text-sm text-white outline-none transition-all"
                       style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)">
            </div>
            ...
            <div>
                ...
                <textarea name="biography" rows="4" required
                          class="w-full px-4 py-2.5 rounded-xl text-sm text-white resize-none outline-none transition-all"
                          style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)">{{ old('biography', $user->biography) }}</textarea>
            </div>

            <div style="border-top:1px solid rgba(255,255,255,0.07)" class="pt-5 space-y-4">
                <p class="text-xs text-slate-400 uppercase tracking-wider font-medium">Changer la photo / mot de passe</p>
                <div>
